                        ...
                        <div class="card">
                            <div class="card-header border-0 pt-6">
                                <div class="card-title">
                                    <div class="d-flex align-items-center position-relative my-1">
                                        ...
                                        <input type="text" data-kt-user-table-filter="search" class="form-control form-control-solid w-250px ps-14" placeholder="Rechercher livraison" />
                                    </div>
                                </div>
                            ...
                            <div class="card-body pt-3">
                                <div class="table-responsive">
                                    <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_table_livraisons">
                                        <thead>
                                            <tr class="text-start text-muted fw-bolder fs-7 text-uppercase gs-0">
                                                ...
                                                <th class="min-w-175px">ID</th>
                                                <th class="min-w-175px">livraison Commandé</th>
                                                <th class="min-w-175px">Date de Livraison</th>
                                                ...
                                                <tr>
                                                    <td>
                                                        <div class="form-check form-check-sm form-check-custom form-check-solid">
                                                            <input class="form-check-input" type="checkbox" value="{{ $livraison->id }}" />
                                                        </div>
                                                    </td>
                                                    <td>{{ $livraison->id }}</td>
                                                    <td>
                                                        @if($livraison->livraisonCommande && $livraison->livraisonCommande->livraison)
                                                            {{ $livraison->livraisonCommande->livraison->nom ?? 'N/A' }} - 
                                                            {{ $livraison->livraisonCommande->commande->reference ?? 'N/A' }}
                                                        @else
                                                            N/A
                                                        @endif
                                                    </td>
                                                    ...
                                                    <td class="text-end">
                                                        <div class="btn-group">
                                                            ...
                                                            <ul class="dropdown-menu">
                                                                ...
                                                                <li>
                                                                    <a class="dropdown-item" href="#" data-bs-toggle="modal" data-bs-target="#deleteModal{{ $livraison->id }}">
              ...
              <div class="d-flex justify-content-between align-items-center flex-wrap mt-5">
                <div class="d-flex align-items-center py-3">
                    <span class="text-muted fs-7 fw-bold">
                        Affichage de {{ $livraisons->firstItem() }} à {{ $livraisons->lastItem() }} sur {{ $livraisons->total() }} entrées
                    </span>
                </div>
                <div class="d-flex flex-wrap py-3">
                    <ul class="pagination">
                        <!-- Premier lien -->
                        <li class="page-item {{ $livraisons->onFirstPage() ? 'disabled' : '' }}">
                            <a href="{{ $livraisons->url(1) }}" class="page-link" aria-label="First">
                                <span aria-hidden="true">&laquo;&laquo;</span>
                            </a>
                        </li>
                        <!-- Lien précédent -->
                        <li class="page-item {{ $livraisons->onFirstPage() ? 'disabled' : '' }}">
                            <a href="{{ $livraisons->previousPageUrl() }}" class="page-link" aria-label="Previous">
                                <span aria-hidden="true">&laquo;</span>
                            </a>
                        </li>
                        <!-- Liens des pages -->
                        @foreach ($livraisons->getUrlRange(max(1, $livraisons->currentPage() - 2), min($livraisons->lastPage(), $livraisons->currentPage() + 2)) as $page => $url)
                            <li class="page-item {{ $page == $livraisons->currentPage() ? 'active' : '' }}">
                                <a href="{{ $url }}" class="page-link">{{ $page }}</a>
                            </li>
                        @endforeach
                        <!-- Lien suivant -->
                        <li class="page-item {{ !$livraisons->hasMorePages() ? 'disabled' : '' }}">
                            <a href="{{ $livraisons->nextPageUrl() }}" class="page-link" aria-label="Next">
                                <span aria-hidden="true">&raquo;</span>
                            </a>
                        </li>
                        <!-- Dernier lien -->
                        <li class="page-item {{ !$livraisons->hasMorePages() ? 'disabled' : '' }}">
                            <a href="{{ $livraisons->url($livraisons->lastPage()) }}" class="page-link" aria-label="Last">
